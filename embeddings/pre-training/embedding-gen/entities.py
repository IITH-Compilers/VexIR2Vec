...
class entities:
    ...
    def processExpTriplet(self, triplet, tString="", prevOpc="", rec=False):
        ...
        if opc is not None:
            ...
            for arg in args:
                if isinstance(arg, str):
                    ...
                    tString = self.processExpTriplet(arg, tString, opc, True)
        return tString

    def processStmtTriplet(self, triplet, tString=""):  # handle next
        opc = triplet["opc"]
        loc = triplet["loc"]
        rhs = triplet["rhs"]
        ty = triplet["type"]
        ...
        if opc != "":
            ...
            if rhs != "":
                if isinstance(rhs, str):
                    ...
                    for r in rhs:
                        tString += ";" + opc + ";" + r["opc"]
                        tString = self.processStmtTriplet(r, tString)
            if ty != "":
                tString += ";" + opc + ";" + ty
            for arg in args:
                if isinstance(arg, str):
                    ...
                    tString += ";" + opc + ";" + arg["opc"]
                    tString += self.processExpTriplet(arg, "", opc)
        return tString

    def processFreq(self, sts, freq_dict, windowSize, stride):
        for index in range(0, len(sts), stride):
            triplet_key = ""
            for i in range(windowSize):
                if index + i >= len(sts):
                    break
                    # Stop at IRSB boundary
                if type(sts[index + i]) != dict:
                    break
                triplet_key += self.processStmtTriplet(sts[index + i])
            if not triplet_key:
                continue
            if triplet_key not in freq_dict.keys():
                freq_dict[triplet_key] = 0
            freq_dict[triplet_key] += 1
        return

    def processFunc(self, func, freq_dict=None, windowSize=1, stride=1):
        sts = []
        for block in func.blocks:
            if block.size > 0:
                sts.extend(self.processSB(block))
        if freq_dict is not None:
            return self.processFreq(sts, freq_dict, windowSize, stride)
        return sts
