...
class Tester(object):
    ...
    def runLinkPrediction(self, type_constrain=False):
        self.lib.initTest()
        ...
        for index, [data_head, data_tail] in enumerate(training_range):
            score = self.testOneStep(data_head)
            self.lib.testHead(
                score.__array_interface__["data"][0], index, type_constrain
            )
            score = self.testOneStep(data_tail)
            self.lib.testTail(
                score.__array_interface__["data"][0], index, type_constrain
            )
    ...
    def getBestThreshlod(self, score, ans):
    ...
    def runTripleClassification(self, threshlod=None):
        self.lib.initTest()
        ...
        for index, [pos_ins, neg_ins] in enumerate(training_range):
            res_pos = self.testOneStep(pos_ins)
            ans = ans + [1 for i in range(len(res_pos))]
            score.append(res_pos)

            res_neg = self.testOneStep(neg_ins)
        ...
        if threshlod == None:
            threshlod, _ = self.getBestThreshlod(score, ans)
