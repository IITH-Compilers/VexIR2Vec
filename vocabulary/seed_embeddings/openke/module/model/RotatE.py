...
class RotatE(Model):
    def __init__(self, ent_tot, rel_tot, dim=100, temperature=6.0, epsilon=2.0):
        ...
        self.ent_embedding_range = nn.Parameter(
            torch.Tensor([(self.temperature + self.epsilon) / self.dim_e]),
            requires_grad=False,
        )
        ...
        self.rel_embedding_range = nn.Parameter(
            torch.Tensor([(self.temperature + self.epsilon) / self.dim_r]),
            requires_grad=False,
        )
    ...
    def forward(self, data):
        ...
        score = self.temperature - self.Calc(h, t, r, mode)
        return score
