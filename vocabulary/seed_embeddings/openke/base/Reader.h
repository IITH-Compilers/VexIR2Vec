...
extern "C" void importTestFiles() {
  ...
  Triple *testTmpList;
  testTmpList = (Triple *)calloc(testTotal, sizeof(Triple));
  ...
  for (INT i = 0; i < testTotal; i++) {
    tmp = fscanf(f_kb1, "%ld", &testTmpList[i].h);
    tmp = fscanf(f_kb1, "%ld", &testTmpList[i].t);
    tmp = fscanf(f_kb1, "%ld", &testTmpList[i].r);
    tripleList[i] = testTmpList[i];
  }
  for (INT i = 0; i < trainTotal; i++) {
  ...
  fclose(f_kb2);
  fclose(f_kb3);
  std::sort(testTmpList, testTmpList + testTotal, Triple::cmp_head);
  tmp = testTotal;
  testTotal = 1;
  testList[0] = testTmpList[0];
  for (INT i = 1; i < tmp; i++)
    if (testTmpList[i].h != testTmpList[i - 1].h ||
        testTmpList[i].r != testTmpList[i - 1].r ||
        testTmpList[i].t != testTmpList[i - 1].t) {
      testList[testTotal] = testTmpList[i];
      testTotal++;
    }

  // std::sort(testHead, testHead + testTotal, Triple::cmp_head);
  // std::sort(testTail, testTail + testTotal, Triple::cmp_tail);
  // std::sort(testRel, testRel + testTotal, Triple::cmp_rel);
  ...
  for (INT i = 1; i < tripleTotal; i++)
    if (tripleList[i].h != tripleList[i - 1].h ||
        tripleList[i].r != tripleList[i - 1].r ||
        tripleList[i].t != tripleList[i - 1].t) {
      uniqueTotal++;
    }
  printf("The total of unique triples is %ld.\n", uniqueTotal);
